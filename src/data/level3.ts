import { GameItem } from '../types';

export const LEVEL3_DATA: GameItem[] = [
  {
    id: 'wheel',
    nameRu: 'Колесо',
    nameEn: 'Wheel',
    value: -3500,
    displayValue: '3500 год до н.э.',
    emoji: '🛞',
    imageUri: '/images/wheel_1786533623627.jpg',
    category: 'Изобретения',
    funFact: 'Одно из важнейших изобретений человечества, появилось в Месопотамии.'
  },
  {
    id: 'internet',
    nameRu: 'Интернет (ARPANET)',
    nameEn: 'Internet',
    value: 1969.83,
    displayValue: '1969 год',
    emoji: '🌐',
    imageUri: '/images/internet_1786531865455.jpg',
    category: 'Технологии',
    funFact: 'Первое сообщение было передано между узлами сети ARPANET 29 октября 1969 года.'
  },
  {
    id: 'pyramids',
    nameRu: 'Пирамида Хеопса',
    nameEn: 'Great Pyramid',
    value: -2560,
    displayValue: '2560 год до н.э.',
    emoji: '🔺',
    imageUri: '/images/pyramids_1786533627208.jpg',
    category: 'Архитектура',
    funFact: 'Единственное из Семи чудес света, сохранившееся до наших дней.'
  },
  {
    id: 'iphone',
    nameRu: 'Первый iPhone',
    nameEn: 'First iPhone',
    value: 2007,
    displayValue: '2007 год',
    emoji: '📱',
    imageUri: '/images/smartphone_1786534090123.jpg',
    category: 'Технологии',
    funFact: 'Был представлен Стивом Джобсом 9 января 2007 года.'
  },
  {
    id: 'printing-press',
    nameRu: 'Печатный станок',
    nameEn: 'Printing Press',
    value: 1440,
    displayValue: '1440 год',
    emoji: '🖨️',
    imageUri: '',
    category: 'Изобретения',
    funFact: 'Изобретен Иоганном Гутенбергом, совершил революцию в распространении информации.'
  },
  {
    id: 'trex',
    nameRu: 'Тираннозавр Рекс',
    nameEn: 'T-Rex',
    value: -68000000,
    displayValue: '68 млн лет назад',
    emoji: '🦖',
    imageUri: '/images/trex_1786531894487.jpg',
    category: 'Природа',
    funFact: 'Один из самых крупных наземных хищников всех времен.'
  },
  {
    id: 'cinema',
    nameRu: 'Первый кинофильм',
    nameEn: 'First Movie',
    value: 1895.99,
    displayValue: '1895 год',
    emoji: '🎞️',
    imageUri: '',
    category: 'Искусство',
    funFact: 'Братья Люмьер устроили первый коммерческий киносеанс в Париже 28 декабря 1895 года.'
  },
  {
    id: 'fire',
    nameRu: 'Освоение огня',
    nameEn: 'Control of Fire',
    value: -1000000,
    displayValue: '1 млн лет назад',
    emoji: '🔥',
    imageUri: '/images/candle_flame_1786533824833.jpg',
    category: 'Эволюция',
    funFact: 'Позволило древним людям готовить пищу, согреваться и защищаться от хищников.'
  },
  {
    id: 'bitcoin',
    nameRu: 'Биткоин',
    nameEn: 'Bitcoin',
    value: 2009,
    displayValue: '2009 год',
    emoji: '₿',
    imageUri: '/images/bitcoin_1786531621369.jpg',
    category: 'Технологии',
    funFact: 'Первая в мире криптовалюта, созданная Сатоши Накамото.'
  },
  {
    id: 'moon-landing',
    nameRu: 'Высадка на Луну',
    nameEn: 'Moon Landing',
    value: 1969.55,
    displayValue: '1969 год',
    emoji: '🌕',
    imageUri: '/images/moon_landing_1786531901839.jpg',
    category: 'Космос',
    funFact: 'Нил Армстронг стал первым человеком, ступившим на поверхность Луны, 20 июля 1969 года.'
  },
  { id: 'compass', nameRu: 'Компас', nameEn: 'Compass', value: -200, displayValue: '200 год до н.э.', emoji: '🧭', imageUri: '', category: 'Открытия', funFact: 'Изобретен в Китае.' },
  { id: 'paper', nameRu: 'Бумага', nameEn: 'Paper', value: 105, displayValue: '105 год', emoji: '📄', imageUri: '/images/paper_1786533669760.jpg', category: 'Материалы', funFact: 'Изобретена Цай Лунем в Китае.' },
  { id: 'gunpowder', nameRu: 'Порох', nameEn: 'Gunpowder', value: 850, displayValue: '850 год', emoji: '🎆', imageUri: '/images/gunpowder_1786531940620.jpg', category: 'Материалы', funFact: 'Случайно изобретен даосскими алхимиками.' },
  { id: 'microscope', nameRu: 'Микроскоп', nameEn: 'Microscope', value: 1590, displayValue: '1590 год', emoji: '🔬', imageUri: '/images/microscope_1786531942314.jpg', category: 'Наука', funFact: 'Открыл людям микромир.' },
  { id: 'telescope', nameRu: 'Телескоп', nameEn: 'Telescope', value: 1608, displayValue: '1608 год', emoji: '🔭', imageUri: '', category: 'Наука', funFact: 'Галилео Галилей первым направил его в небо.' },
  { id: 'steam_engine', nameRu: 'Паровой двигатель', nameEn: 'Steam Engine', value: 1712, displayValue: '1712 год', emoji: '🚂', imageUri: '', category: 'Технологии', funFact: 'Дал старт индустриальной революции.' },
  { id: 'photography', nameRu: 'Фотография', nameEn: 'Photography', value: 1826, displayValue: '1826 год', emoji: '📷', imageUri: '/images/photography_1786531968135.jpg', category: 'Искусство', funFact: 'Первое фото делалось 8 часов.' },
  { id: 'radio', nameRu: 'Радио', nameEn: 'Radio', value: 1895.66, displayValue: '1895 год', emoji: '📻', imageUri: '/images/radio_1786531969861.jpg', category: 'Связь', funFact: 'Летом 1895 года Маркони провел первые опыты беспроводной передачи сигнала.' },
  { id: 'television', nameRu: 'Телевидение', nameEn: 'Television', value: 1927, displayValue: '1927 год', emoji: '📺', imageUri: '/images/television_1786531971434.jpg', category: 'Технологии', funFact: 'Сначала было черно-белым.' },
  { id: 'computer', nameRu: 'ЭВМ (Компьютер)', nameEn: 'Computer', value: 1945, displayValue: '1945 год', emoji: '💻', imageUri: '/images/computer_1786531640560.jpg', category: 'Технологии', funFact: 'Первые компьютеры занимали целые комнаты.' },
  { id: 'laser', nameRu: 'Лазер', nameEn: 'Laser', value: 1960, displayValue: '1960 год', emoji: '🔴', imageUri: '/images/laser_1786533698700.jpg', category: 'Технологии', funFact: 'Сейчас используется везде: от указок до хирургии.' },
  { id: 'email', nameRu: 'Электронная почта', nameEn: 'Email', value: 1971, displayValue: '1971 год', emoji: '📧', imageUri: '/images/email_1786531984541.jpg', category: 'Связь', funFact: 'Знак @ был выбран Реем Томлинсоном.' },
  { id: 'pc', nameRu: 'Персональный компьютер', nameEn: 'PC', value: 1974, displayValue: '1974 год', emoji: '🖥️', imageUri: '/images/pc_1786533704894.jpg', category: 'Технологии', funFact: 'Сделал компьютеры доступными дома.' },
  { id: 'social_media', nameRu: 'Соцсети', nameEn: 'Social Media', value: 2004, displayValue: '2004 год', emoji: '📱', imageUri: '/images/social_media_1786532000270.jpg', category: 'Технологии', funFact: 'Изменили способ общения людей.' }
];
